...
2. **For the current script (`demo_evidence_programming...`)**: This is just a standalone demo script with a hardcoded loop. It does not launch the full orchestrator; instead, it manually starts the `mcp-simple-pubmed` server and steps through the verification process itself.
