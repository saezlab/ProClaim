...
| `--top-k` | `5` | Number of retrieved items (S2 abstracts for s2_retrieval; passages for open_scholar). |
...
| Flag | Default | Description |
|------|---------|-------------|
| `--search-backend` | `web` | Search backend: `web` (Serper/DuckDuckGo) or `s2` (Semantic Scholar). |

### Prerequisites

- **All LLM baselines**: An LLM API key recognised by litellm (e.g. `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`).
- **S2Retrieval / OpenScholar**: `S2_API_KEY` (defaults to `""`, anonymous, rate-limited).
- **OpenScholar**: OpenScholar checked out at `<workspace>/OpenScholar` with `.venv` intact.
- **FIRE**: FIRE cloned at `<workspace>/fire` with `.venv` set up. `SERPER_API_KEY` must be set.
- **ACE**: ACE cloned at `<workspace>/ace` (reference only; runs in-process).
- **ReAct**: `SERPER_API_KEY` (optional; falls back to DuckDuckGo via `ddgs`).
