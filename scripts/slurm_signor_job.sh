#!/bin/bash
# =============================================================================
# slurm_signor_job.sh — Self-contained SLURM job script
#
# Starts vLLM in the background on the allocated GPU, waits for it to be ready,
# runs the SIGNOR evaluation for this array task's row chunk, then cleans up.
#
# Environment variables (exported by submit_signor_batch.sh via sbatch):
#   RUN_TAG      — timestamped run identifier (e.g. 20260330_142500)
#   EXTRA_ARGS   — additional args forwarded to run_signor_eval.py (e.g. --reps 1)
...
if [[ "${SINGLE_MODE:-false}" == "true" ]]; then
    OUTPUT_CSV="${PROJECT_ROOT}/results/signor_eval_${RUN_TAG}/results.csv"
else
    OUTPUT_CSV="${PROJECT_ROOT}/results/signor_eval_${RUN_TAG}/results_chunk${TASK_ID}.csv"
fi
...
bash "${PROJECT_ROOT}/scripts/run_signor_batch.sh" ${RUN_ARGS} ${EXTRA_ARGS:-}
