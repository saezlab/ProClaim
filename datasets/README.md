...
Claims are direct interaction statements `{A} directly activates {B}` | `{A} directly inhibits {B}` derived from SIGNOR source-target-effect rows, including flipped variants for negated examples.
