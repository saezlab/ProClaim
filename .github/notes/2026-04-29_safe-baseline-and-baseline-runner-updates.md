...
| `experiments/configs/fire_s2_config.yaml` | Dedicated YAML config for running the new Semantic Scholar-backed FIRE variant. |

**Modified Files**

| File | Changes |
|------|---------|
| `experiments/run_baselines_datasets.py` | Registered the `safe` baseline in `build_baseline()`, extended CLI choices and help text, and added `safe/<backend>/<model>` output directory routing. |
| `experiments/baselines/fire_baseline.py` | Added configurable search backend support so FIRE can use either DuckDuckGo or Semantic Scholar while preserving the existing iterative decision loop. |
...
- Kept SAFE search backend configurable (`web` or `s2`) even though the default config uses web search, which stays closest to the original paper while allowing literature-only ablations.
...
- Added explicit `safe/<backend>/<model>` output subdirectories so SAFE runs do not collide with ReAct outputs.
- Reused the existing `fire` baseline name for both FIRE variants and made the backend configurable through `--search-backend`, then exposed the S2-backed run as a separate config/launcher target instead of duplicating baseline code.
...
- Removed FIRE's hard-coded web dependency by routing retrieval through a backend dispatcher, which enables a literature-only Semantic Scholar variant without duplicating the FIRE control loop.